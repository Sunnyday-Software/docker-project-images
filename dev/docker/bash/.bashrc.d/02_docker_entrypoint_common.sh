...
function docker_entrypoint_tmux {
  ...
  if ! command -v tmux >/dev/null 2>&1; then
    echo "Errore: tmux non trovato nel PATH." >&2
    return 127
  fi
  ...
  local cmd=("$@")
  if [ ${#cmd[@]} -eq 0 ]; then
    echo "Nessun comando specificato. Avvio shell interattiva nella sessione tmux '${session_name}'." >&2
    cmd=("bash" "-l")
  fi
  ...
  echo "Comando inviato alla sessione tmux '${session_name}'."
  echo "Per collegarti: tmux attach -t ${session_name}"
