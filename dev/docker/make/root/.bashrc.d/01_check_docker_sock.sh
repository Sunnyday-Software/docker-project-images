...
if log_is_debug_enabled; then
  ...
  if [[ "$DPM_DOCKER_HOST" == unix://* ]]; then
    ...
    if [ -f /.dockerenv ]; then
      log_debug "   ✓ Sì (file /.dockerenv esiste)"
    elif grep -q docker /proc/1/cgroup 2>/dev/null; then
      log_debug "   ✓ Sì (cgroup indica docker)"
    else
      log_debug "   ? Forse no"
    fi
    echo

    log_debug "Fine diagnostica"

  else
    echo "Niente da testare pe il socket $DPM_DOCKER_HOST"
  fi
fi